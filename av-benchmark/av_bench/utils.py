...
def unroll_paired_dict_with_key(gt_d: dict,
    ...
    for sample_name, features in paired_features.items():
        if sample_name not in gt_features:
            print(f'1111111111111111 Sample {sample_name} not found in ground truth.')
            continue
    ...
    print("sssssssssssssssssssssss: ", gt_feat_list.shape, len(paired_feat_list), paired_feat_list[0].shape)
...
def unroll_paired_dict(gt_dict: dict,
    ...
    print("aaaaaaaaaaaaaaaaaaaaaaa: ", len(gt_dict), len(pred_dict))
    ...
    for key, sample_name in pred_keys_to_sample.items():
        if sample_name in unpaired_samples:
            print(f'2222222222222222Sample {sample_name} not found in ground truth.')
            continue
